...
export function useCrewAI() {
  ...
  const executeWorkflow = useCallback(async (
    ...
    try {
      ...
      // Use the command from parameters or construct a default command
      const command = parameters.command || `Execute ${workflowType} task`;

      // Use the same endpoint as executeCommand
      const response = await fetch('http://localhost:8000/process-command', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
